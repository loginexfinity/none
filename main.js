...
((win, doc, und, vars) => {
    ...
    Themify = {
        ...
        triggerEvent(el, type, params, isNotCustom) {
            ...
            if (isNotCustom === true || type === 'click' || type === 'submit' || type === 'input' || type === 'resize' || (type === 'change' && !params) || type.indexOf('pointer') === 0 || type.indexOf('touch') === 0 || type.indexOf('mouse') === 0) {
                ...
                Object.defineProperty(ev, 'target', {
                    value: params.target || el,
                    enumerable: true
                });
            } else {
                ev = new win.CustomEvent(type, {
                    detail: params
                });
            }
            el.dispatchEvent(ev);
        ...
        updateQueryString(d, a, b) {
            b || (b = win.location.href);
            const e = new URL(b, win.location),
                f = e.searchParams;
            null === a ? f.delete(d) : f.set(d, a);
            let g = f.toString();
            return '' !== g && (g = '?' + g), b.split('?')[0] + g + e.hash;
        },
            ...
            const windowLoad = () => {
                ...
                requestAnimationFrame(() => {
                    if (this.urlArgs !== null) {
                        this.urlArgs = '&' + (new URLSearchParams({
                            media: this.urlArgs
                        })).toString();
                    }
                ...
                if (needToLoad.length > 0) {
                    ...
                    setTimeout(() => {
                        this.fetch({
                            ...
                            icons: JSON.stringify(needToLoad)
                        }, null, {
                            credentials: 'omit'
                        }).then(res => {
                                        ...
                                        .finally(() => {
                                            requestAnimationFrame(() => {
                                                ...
                                                if (img.classList.contains('tf_svg_lazy')) {
                                                    img.tfOn('transitionend', svg_callback, {
                                                        once: true,
                                                        passive: true
                                                    });
                                                    requestAnimationFrame(() => {
                ...
                const self = this,
                    ...
                    closeDropdown = function(e) {
                        ...
                        if (e.target && !el[0].parentNode.contains(e.target)) {
                            ...
                            doc.tfOn('touchstart', closeDropdown.bind(el), {
                                once: true
                            });
                        }
                    };
            ...
            if (items[0] !== und) {
                ...
                if (embeds[0] !== und) {
                    ...
                    for (let i = embeds.length - 1; i > -1; --i) {
                        ...
                        win.wp.receiveEmbedMessage({
                            data: {
                                message: 'height',
                                value: this.h,
                                secret: secret
                            },
                            source: embeds[i].contentWindow
                        });
                    }
                }
            }
